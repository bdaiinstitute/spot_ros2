...
class WalkForward(Node):
    def __init__(self, name) -> None:
        ...
        self._tf_listener = TFListenerWrapper(
            "walk_forward_tf", wait_for_transform=[name + 
"/" + BODY_FRAME_NAME, name + "/" + VISION_FRAME_NAME]
        )
        self._robot = SimpleSpotCommander(name)
        self._robot_command_client = ActionClientWrapper(RobotCommand, "robot_command", "walk_forward_action_node", namespace=name)
    ...
    def walk_forward_with_world_frame_goal(self) -> None:
        ...
        world_t_robot = self._tf_listener.lookup_a_tform_b(
            "Eddie/" + VISION_FRAME_NAME, "Eddie/" + BODY_FRAME_NAME
