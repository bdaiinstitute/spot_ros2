...
void StateStreamingHandler::get_states(JointStates& joint_states, std::vector<int>& foot_states) {
  // lock so that read/write doesn't happen at the same time
  const std::lock_guard<std::mutex> lock(mutex_);
  ...
  // Fill in foot statues (0: unknown; 1: contact; 2: no contact)
...
hardware_interface::return_type SpotHardware::read(const rclcpp::Time& /*time*/, const rclcpp::Duration& /*period*/) {
  state_streaming_handler_.get_states(joint_states_, foot_states_);
