...
flower, completing its pattern every 10 seconds.
Concurrently, the robot's torso executes a rotation dictated by a sine wave,
also with a 10-second periodicity.
...
Simultaneously, the robot executes a curved path with its arm while performing
a rotational movement with its torso.
...
trajectories for the robot's gripper.
