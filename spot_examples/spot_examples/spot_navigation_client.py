...
#         goal_msg.navigate_to = args.navigate_to
...
#     parser.add_argument("--navigate_to", type=str, default="")
...
#     navigate_to_waypoint.initialize_robot()
...
class NavigateToWaypoint:
    ...
    def list_waypoints(self, upload_path):
        ...
        if len(response.waypoint_ids) > 0:
            ...
            self.waypoints = response.waypoint_ids
            return self.waypoints
        else:
            self._logger.error("ListGraph failed: " + response.waypoint_ids)
            return None
    ...
    def navigate_to_waypoint(self, args):
        self._logger.info("Navigate to waypoint")
        goal_msg = NavigateTo.Goal()
        goal_msg.navigate_to = self.waypoints[args.navigate_to]
        self._robot_navigation_client.send_goal_and_wait("navigate_to", goal_msg)
...
def main(args: argparse.Namespace) -> int:
    navigate_to_waypoint = NavigateToWaypoint(args.robot, main.node)
    if navigate_to_waypoint.list_waypoints(args.upload_path) is None:
        return 0
    navigate_to_waypoint.initialize_robot()
    time.sleep(2)
    position_initialized = True
