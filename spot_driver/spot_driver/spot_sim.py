...
class SpotSim:
    ...
    def __init__(self, username, password, hostname, robot_name, node, logger, start_estop, estop_timeout=9.0,
        ...
        self._robot_state_task = AsyncRobotState(self._logger, max(0.0, self._rates.get("robot_state", 0.0)), self._callbacks.get("robot_state", None))
    ...
    def self_right(self):
        self.logger.info("spotSim self_right")
    ...
    def stand(self, monitor_command=True):
        ...
        self.logger.info("spotSim Stand")
        response = self.power_on(allow_already_powered=True)
        if not response[0]:
            return response
    ...
    def power_on(self, allow_already_powered=False):
        self.logger.info("spotSim power_on")
        return True, "Success"
    ...
    def get_frontright_rgb_image(self):
        try:
            print("*** get_frontright_rgb_image")
            return self._image_client.get_image([build_image_request(
    ...
    def get_images(self, image_requests: list[image_pb2.ImageRequest]) -> Optional[ImageBundle]:
        return ImageBundle(
            frontleft=image_pb2.ImageResponse(),
            frontright=image_pb2.ImageResponse(),
            left=image_pb2.ImageResponse(),
            right=image_pb2.ImageResponse(),
            back=image_pb2.ImageResponse(),
        )
