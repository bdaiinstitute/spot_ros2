...
import rclpy.time
import tf2_ros
from bosdyn.api import geometry_pb2, manipulation_api_pb2, robot_command_pb2, trajectory_pb2, world_object_pb2
...
from geometry_msgs.msg import Pose, PoseStamped, Twist, TwistWithCovarianceStamped
...
from spot_msgs.action import Manipulation, NavigateTo, RobotCommand, Trajectory
...
class SpotROS:
    ...
    def handle_robot_command(self, goal_handle):
        ...
        result.success = self._goal_complete(feedback) is True
    ...
    def handle_graph_nav_get_localization_pose(self, request, response):
        ...
        if response.success:
            self.node.get_logger().info("GraphNav localization pose received")
        return response
    ...
    def handle_graph_nav_upload_graph(self, request, response):
        try:
            ...
            self.node.get_logger().info("Uploaded")
            response.success = True
            response.message = "Success"
        except Exception as e:
            self.node.get_logger().error(f"Exception Error:{e}; \n {traceback.format_exc()}")
...
def main(args=None):
    ...
    spot_ros.tf_name_kinematic_odom = node.declare_parameter("tf_name_kinematic_odom", frame_prefix + "odom")
    ...
    if (
        ...
        node.get_logger().error(
            'rosparam "mode_parent_odom_tf" should be "' + frame_prefix + 'odom" or ' + frame_prefix + '"vision".'
        )
        return
    ...
    if spot_ros.name == MOCK_HOSTNAME:
        ...
        if spot_ros.publish_rgb.value:
            for camera_name in spot_ros.cameras_used.value:
                setattr(
                    spot_ros, f"{camera_name}_image_pub", node.create_publisher(Image, f"camera/{camera_name}/image", 1)
                )
                setattr(
                    spot_ros,
                    f"{camera_name}_image_info_pub",
                    node.create_publisher(CameraInfo, f"camera/{camera_name}/camera_info", 1),
                )
        ...
        if spot_ros.publish_depth.value:
            for camera_name in spot_ros.cameras_used.value:
                setattr(
                    spot_ros, f"{camera_name}_depth_pub", node.create_publisher(Image, f"depth/{camera_name}/image", 1)
                )
                setattr(
                    spot_ros,
                    f"{camera_name}_depth_info_pub",
                    node.create_publisher(CameraInfo, f"depth/{camera_name}/camera_info", 1),
                )
        ...
        if spot_ros.publish_depth_registered.value:
            for camera_name in spot_ros.cameras_used.value:
                setattr(
                    ...
                    node.create_publisher(Image, f"depth_registered/{camera_name}/image", 1),
                )
                setattr(
                    spot_ros,
                    f"{camera_name}_depth_registered_info_pub",
                    node.create_publisher(CameraInfo, f"depth_registered/{camera_name}/camera_info", 1),
                )
        ...
        if spot_ros.publish_graph_nav_pose.value:
            ...
            spot_ros.graph_nav_pose_pub = node.create_publisher(PoseStamped, "graph_nav/body_pose", 1)
        ...
        node.create_service(
            ...
            lambda request, response: spot_ros.service_wrapper(
                "clear_behavior_fault", spot_ros.handle_clear_behavior_fault, request, response
            ),
            callback_group=spot_ros.group,
        )
        node.create_service(
        ...
        node.create_service(ListWorldObjects, "list_world_objects", spot_ros.handle_list_world_objects)
        ...
        spot_ros.navigate_as = ActionServer(
            node, NavigateTo, "navigate_to", spot_ros.handle_navigate_to, callback_group=spot_ros.group
        )
        # spot_ros.navigate_as.start() # As is online

        spot_ros.trajectory_server = ActionServer(
            node, Trajectory, "trajectory", spot_ros.handle_trajectory, callback_group=spot_ros.group
        )
        # spot_ros.trajectory_server.start()

        spot_ros.robot_command_server = SingleGoalActionServer(
            node, RobotCommand, "robot_command", spot_ros.handle_robot_command, callback_group=spot_ros.group
        )
        if has_arm:
            spot_ros.manipulation_server = ActionServer(
                node, Manipulation, "manipulation", spot_ros.handle_manipulation_command, callback_group=spot_ros.group
            )
