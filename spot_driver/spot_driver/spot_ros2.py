...
class SpotROS:
        ...
        try:
            ...
            if not state.localization.waypoint_id:
                self.node.get_logger().warning(
                    "robot is not localized; Please upload graph and localize."
                )
                return
            ...
            self.graph_nav_pose_transform_broadcaster.sendTransform(
                seed_t_body_trans_msg
            )
    ...
    def publish_camera_images_callback(self):
        result = self.spot_wrapper.get_images_by_cameras(
            [
                CameraSource(camera_name, ["visual"])
    ...
    def publish_depth_images_callback(self):
        result = self.spot_wrapper.get_images_by_cameras(
            [
                CameraSource(camera_name, ["depth"])
                for camera_name in self.cameras_used.value
            ]
        )
        for image_entry in result:
    ...
    def publish_depth_registered_images_callback(self):
        result = self.spot_wrapper.get_images_by_cameras(
            [
                CameraSource(camera_name, ["depth_registered"])
                for camera_name in self.cameras_used.value
            ]
        )
        for image_entry in result:
            ...
            depth_registered_pub = getattr(
                self, f"{image_entry.camera_name}_depth_registered_pub"
            )
            depth_registered_info_pub = getattr(
                self, f"{image_entry.camera_name}_depth_registered_info_pub"
            )
    ...
    def service_wrapper(self, name, handler, request, response):
        if self.spot_wrapper is None:
            self.node.get_logger().info(
                "Mock mode: service "
                + name
                + " successfully called with request "
                + str(request)
            )
    ...
    def handle_clear_behavior_fault(self, request, response):
        ...
        response.success, response.message = self.spot_wrapper.clear_behavior_fault(
            request.id
        )
        return response
    ...
    def _goal_complete(self, feedback):
        ...
        if (
            feedback.command.command_choice
            == feedback.command.COMMAND_FULL_BODY_FEEDBACK_SET
        ):
            if (
            ...
            if (
                ...
                if (
                    ...
                    == feedback.command.full_body_feedback.feedback.constrained_manipulation_feedback.status.STATUS_RUNNING
                ):
                    return IN_PROGRESS
                return FAILED
            else:
            ...
            feedback.command.command_choice
            == feedback.command.COMMAND_SYNCHRONIZED_FEEDBACK_SET
        ):
            ...
            if sync_feedback.arm_command_feedback_is_set == True:
                ...
                if (
                    arm_feedback.status.value
                    == arm_feedback.status.STATUS_COMMAND_OVERRIDDEN
                    or arm_feedback.status.value
                    == arm_feedback.status.STATUS_COMMAND_TIMED_OUT
                    or arm_feedback.status.value
                    == arm_feedback.status.STATUS_ROBOT_FROZEN
                    or arm_feedback.status.value
                    == arm_feedback.status.STATUS_INCOMPATIBLE_HARDWARE
                ):
                    return FAILED
                if (
                    arm_feedback.feedback.feedback_choice
                    == arm_feedback.feedback.FEEDBACK_ARM_CARTESIAN_FEEDBACK_SET
                ):
                    if (
                    ...
                    arm_feedback.feedback.feedback_choice
                    == arm_feedback.feedback.FEEDBACK_ARM_JOINT_MOVE_FEEDBACK_SET
                ):
                    if (
                    ...
                    arm_feedback.feedback.feedback_choice
                    == arm_feedback.feedback.FEEDBACK_ARM_VELOCITY_FEEDBACK_SET
                ):
                    self.node.get_logger().warn(
                        "WARNING: ArmVelocityCommand provides no feedback"
                    )
                    pass  # May return SUCCESS below
                elif (
                    arm_feedback.feedback.feedback_choice
                    == arm_feedback.feedback.FEEDBACK_ARM_GAZE_FEEDBACK_SET
                ):
                    if (
                    ...
                    arm_feedback.feedback.feedback_choice
                    == arm_feedback.feedback.FEEDBACK_ARM_STOP_FEEDBACK_SET
                ):
                    self.node.get_logger().warn(
                        "WARNING: Stop command provides no feedback"
                    )
                    pass  # May return SUCCESS below
                elif (
                    arm_feedback.feedback.feedback_choice
                    == arm_feedback.feedback.FEEDBACK_ARM_DRAG_FEEDBACK_SET
                ):
                    if (
                    ...
                    arm_feedback.feedback.feedback_choice
                    == arm_feedback.feedback.FEEDBACK_ARM_IMPEDANCE_FEEDBACK_SET
                ):
                    self.node.get_logger().warn(
                        "WARNING: ArmImpedanceCommand provides no feedback"
                    )
                    pass  # May return SUCCESS below
                else:
            ...
            if sync_feedback.mobility_command_feedback_is_set == True:
                ...
                if (
                    mob_feedback.status.value
                    == mob_feedback.status.STATUS_COMMAND_OVERRIDDEN
                    or mob_feedback.status.value
                    == mob_feedback.status.STATUS_COMMAND_TIMED_OUT
                    or mob_feedback.status.value
                    == mob_feedback.status.STATUS_ROBOT_FROZEN
                    or mob_feedback.status.value
                    == mob_feedback.status.STATUS_INCOMPATIBLE_HARDWARE
                ):
                    return FAILED
                if (
                    mob_feedback.feedback.feedback_choice
                    == mob_feedback.feedback.FEEDBACK_SE2_TRAJECTORY_FEEDBACK_SET
                ):
                    if (
                    ...
                    mob_feedback.feedback.feedback_choice
                    == mob_feedback.feedback.FEEDBACK_SE2_VELOCITY_FEEDBACK_SET
                ):
                    self.node.get_logger().warn(
                        "WARNING: Planar velocity commands provide no feedback"
                    )
                    pass  # May return SUCCESS below
                elif (
                    mob_feedback.feedback.feedback_choice
                    == mob_feedback.feedback.FEEDBACK_SIT_FEEDBACK_SET
                ):
                    if (
                    ...
                    mob_feedback.feedback.feedback_choice
                    == mob_feedback.feedback.FEEDBACK_STAND_FEEDBACK_SET
                ):
                    if (
                    ...
                    mob_feedback.feedback.feedback_choice
                    == mob_feedback.feedback.FEEDBACK_STANCE_FEEDBACK_SET
                ):
                    if (
                    ...
                    mob_feedback.feedback.feedback_choice
                    == mob_feedback.feedback.FEEDBACK_STOP_FEEDBACK_SET
                ):
                    self.node.get_logger().warn(
                        "WARNING: Stop command provides no feedback"
                    )
                    ...
                    mob_feedback.feedback.feedback_choice
                    == mob_feedback.feedback.FEEDBACK_FOLLOW_ARM_FEEDBACK_SET
                ):
                    self.node.get_logger().warn(
                        "WARNING: FollowArmCommand provides no feedback"
                    )
                    pass  # May return SUCCESS below
                else:
            ...
            if sync_feedback.gripper_command_feedback_is_set == True:
                ...
                if (
                    grip_feedback.status.value
                    == grip_feedback.status.STATUS_COMMAND_OVERRIDDEN
                    or grip_feedback.status.value
                    == grip_feedback.status.STATUS_COMMAND_TIMED_OUT
                    or grip_feedback.status.value
                    == grip_feedback.status.STATUS_ROBOT_FROZEN
                    or grip_feedback.status.value
                    == grip_feedback.status.STATUS_INCOMPATIBLE_HARDWARE
                ):
                    return FAILED
                if (
                    grip_feedback.command.command_choice
                    == grip_feedback.command.COMMAND_CLAW_GRIPPER_FEEDBACK_SET
                ):
                    if (
    ...
    def _get_robot_command_feedback(self, goal_id):
        ...
        if goal_id is None:
            ...
            feedback.command.synchronized_feedback.mobility_command_feedback.feedback.feedback_choice = (
                feedback.command.synchronized_feedback.mobility_command_feedback.feedback.FEEDBACK_SE2_TRAJECTORY_FEEDBACK_SET
            )
            if self._wait_for_goal.at_goal:
                feedback.command.synchronized_feedback.mobility_command_feedback.feedback.se2_trajectory_feedback.status.value = (
                    feedback.command.synchronized_feedback.mobility_command_feedback.feedback.se2_trajectory_feedback.status.STATUS_AT_GOAL
                )
            else:
                feedback.command.synchronized_feedback.mobility_command_feedback.feedback.se2_trajectory_feedback.status.value = (
                    feedback.command.synchronized_feedback.mobility_command_feedback.feedback.se2_trajectory_feedback.status.STATUS_GOING_TO_GOAL
                )
        else:
    ...
    def handle_robot_command(self, goal_handle):
        ...
        while (
            ...
            time.sleep(
                0.1
            )  # don't use rate here because we're already in a single thread
    ...
    def handle_manipulation_command(self, goal_handle):
        ...
        self.node.get_logger().debug(
            "I'm a function that handles request to the manipulation api!"
        )

        ros_command = goal_handle.request.command
        proto_command = manipulation_api_pb2.ManipulationApiRequest()
        conv.convert_bosdyn_msgs_manipulation_api_request_to_proto(
            ros_command, proto_command
        )
        self._wait_for_goal = None
        if not self.spot_wrapper:
            self._wait_for_goal = WaitForGoal(self.node.get_clock(), 2.0)
            ...
            success, err_msg, goal_id = self.spot_wrapper.manipulation_command(
                proto_command
            )
            if not success:
                raise Exception(err_msg)
    ...
    def handle_trajectory(self, goal_handle):
        ...
        try:
            while (
                rclpy.ok() and not self.spot_wrapper.at_goal and goal_handle.is_active
            ):
    ...
    def handle_graph_nav_get_localization_pose(self, request, response):
        try:
            ...
            if not state.localization.waypoint_id:
                response.success = False
                response.message = (
                    "The robot is currently not localized to the map; Please localize."
                )
    ...
    def handle_graph_nav_set_localization(self, request, response):
        try:
            if request.method == "fiducial":
                ...
                self.spot_wrapper._set_initial_localization_waypoint(
                    [request.waypoint_id]
                )
                response.success = True
                response.message = "Success"
                ...
                response.success = False
                response.message = (
                    f"Invalid localization method {request.method}."
        ...
        if response.success:
            self.node.get_logger().info(
                f"Successfully set GraphNav localization. Method: {request.method}"
            )
        return response
    ...
    def handle_graph_nav_upload_graph(self, request, response):
        try:
            self.node.get_logger().info(
                f"Uploading GraphNav map: {request.upload_filepath}"
            )
    ...
    def handle_list_graph(self, request, response):
        ...
        try:
            ...
            self.spot_wrapper._clear_graph()
            self.spot_wrapper._upload_graph_and_snapshots(request.upload_filepath)
            response.waypoint_ids = self.spot_wrapper.list_graph(
                request.upload_filepath
            )
    ...
    def handle_list_world_objects(self, request, response):
        ...
        try:
            ...
            print(
                "In handling list world objects, exception was",
    ...
    def _handle_list_world_objects(self, request, response):
        ...
        if request.request.timestamp_filter_is_set:
            time_start_point = (
                request.request.timestamp_filter.sec
        ...
        if not self.spot_wrapper:
            ...
            proto_response = self.spot_wrapper.list_world_objects(
                object_types, time_start_point
            )
        conv.convert_proto_to_bosdyn_msgs_list_world_object_response(
...
def main(args=None):
    ...
    spot_ros.username = get_from_env_and_fall_back_to_param(
        "BOSDYN_CLIENT_USERNAME", node, "username", "user"
    )
    spot_ros.password = get_from_env_and_fall_back_to_param(
        "BOSDYN_CLIENT_PASSWORD", node, "password", "password"
    )
    spot_ros.ip = get_from_env_and_fall_back_to_param(
        "SPOT_IP", node, "hostname", "10.0.0.3"
    )

    spot_ros.camera_static_transform_broadcaster = tf2_ros.StaticTransformBroadcaster(
        node
    )
    ...
    spot_ros.mode_parent_odom_tf = node.declare_parameter(
        "mode_parent_odom_tf", frame_prefix + "odom"
    )  # 'vision' or 'odom'
    spot_ros.tf_name_kinematic_odom = node.declare_parameter(
        "tf_name_kinematic_odom", frame_prefix + "odom"
    )
    spot_ros.tf_name_raw_kinematic = frame_prefix + "odom"
    spot_ros.tf_name_vision_odom = node.declare_parameter(
        "tf_name_vision_odom", frame_prefix + "vision"
    )
    spot_ros.tf_name_raw_vision = frame_prefix + "vision"

    if (
        spot_ros.mode_parent_odom_tf.value != spot_ros.tf_name_raw_kinematic
        and spot_ros.mode_parent_odom_tf.value != spot_ros.tf_name_raw_vision
    ):
        node.get_logger().error(
            'rosparam "mode_parent_odom_tf" should be "'
            + frame_prefix
            + 'odom" or '
            + frame_prefix
            + '"vision".'
    ...
    spot_ros.wrapper_logger = rcutils_logger.RcutilsLogger(
        name=f"{name_with_dot}spot_wrapper"
    )
    ...
    if spot_ros.name == MOCK_HOSTNAME:
        ...
        spot_ros.spot_wrapper = SpotWrapper(
            spot_ros.username,
            spot_ros.password,
            spot_ros.ip,
            spot_ros.name,
            spot_ros.wrapper_logger,
            spot_ros.start_estop.value,
            spot_ros.estop_timeout.value,
            spot_ros.rates,
            spot_ros.callbacks,
            spot_ros.use_take_lease,
            spot_ros.get_lease_on_action,
            spot_ros.continually_try_stand,
        )
        ...
        if spot_ros.publish_rgb.value:
            for camera_name in spot_ros.cameras_used.value:
                setattr(
                    ...
                    node.create_publisher(Image, f"camera/{camera_name}/image", 1),
                )
                setattr(
                    spot_ros,
                    f"{camera_name}_image_info_pub",
                    node.create_publisher(
                        CameraInfo, f"camera/{camera_name}/camera_info", 1
                    ),
                )
        ...
        if spot_ros.publish_depth.value:
            for camera_name in spot_ros.cameras_used.value:
                setattr(
                    spot_ros,
                    f"{camera_name}_depth_pub",
                    node.create_publisher(Image, f"depth/{camera_name}/image", 1),
                )
                setattr(
                    spot_ros,
                    f"{camera_name}_depth_info_pub",
                    node.create_publisher(
                        CameraInfo, f"depth/{camera_name}/camera_info", 1
                    ),
                )
        ...
        if spot_ros.publish_depth_registered.value:
            for camera_name in spot_ros.cameras_used.value:
                setattr(
                    ...
                    node.create_publisher(
                        Image, f"depth_registered/{camera_name}/image", 1
                    ),
                )
                setattr(
                    spot_ros,
                    ...
                    node.create_publisher(
                        CameraInfo, f"depth_registered/{camera_name}/camera_info", 1
                    ),
                )
        ...
        if spot_ros.publish_graph_nav_pose.value:
            ...
            spot_ros.graph_nav_pose_pub = node.create_publisher(
                PoseStamped, f"graph_nav/body_pose", 1
            )
            spot_ros.graph_nav_pose_transform_broadcaster = (
                tf2_ros.StaticTransformBroadcaster(node)
            )
        ...
        spot_ros.odom_twist_pub = node.create_publisher(
            TwistWithCovarianceStamped, "odometry/twist", 1
        )
        ...
        spot_ros.battery_pub = node.create_publisher(
            BatteryStateArray, "status/battery_states", 1
        )
        spot_ros.behavior_faults_pub = node.create_publisher(
            BehaviorFaultState, "status/behavior_faults", 1
        )
        spot_ros.system_faults_pub = node.create_publisher(
            SystemFaultState, "status/system_faults", 1
        )

        spot_ros.feedback_pub = node.create_publisher(Feedback, "status/feedback", 1)

        spot_ros.mobility_params_pub = node.create_publisher(
            MobilityParams, "status/mobility_params", 1
        )

        node.create_subscription(
            Twist, "cmd_vel", spot_ros.cmdVelCallback, 1, callback_group=spot_ros.group
        )
        node.create_subscription(
            Pose,
            "body_pose",
            spot_ros.bodyPoseCallback,
            1,
        ...
        node.create_service(
            ...
            lambda request, response: spot_ros.service_wrapper(
                "claim", spot_ros.handle_claim, request, response
            ),
            callback_group=spot_ros.group,
        )
        node.create_service(
            ...
            "release",
            lambda request, response: spot_ros.service_wrapper(
                "release", spot_ros.handle_release, request, response
            ),
            callback_group=spot_ros.group,
        )
        node.create_service(
            ...
            "stop",
            lambda request, response: spot_ros.service_wrapper(
                "stop", spot_ros.handle_stop, request, response
            ),
            callback_group=spot_ros.group,
        )
        node.create_service(
            ...
            "self_right",
            lambda request, response: spot_ros.service_wrapper(
                "self_right", spot_ros.handle_self_right, request, response
            ),
            callback_group=spot_ros.group,
        )
        node.create_service(
            ...
            "sit",
            lambda request, response: spot_ros.service_wrapper(
                "sit", spot_ros.handle_sit, request, response
            ),
            callback_group=spot_ros.group,
        )
        node.create_service(
            ...
            "stand",
            lambda request, response: spot_ros.service_wrapper(
                "stand", spot_ros.handle_stand, request, response
            ),
            callback_group=spot_ros.group,
        )
        node.create_service(
            ...
            "rollover",
            lambda request, response: spot_ros.service_wrapper(
                "rollover", spot_ros.handle_rollover, request, response
            ),
            callback_group=spot_ros.group,
        )
        node.create_service(
            ...
            "power_on",
            lambda request, response: spot_ros.service_wrapper(
                "power_on", spot_ros.handle_power_on, request, response
            ),
            callback_group=spot_ros.group,
        )
        node.create_service(
            ...
            "power_off",
            lambda request, response: spot_ros.service_wrapper(
                "power_off", spot_ros.handle_safe_power_off, request, response
            ),
            callback_group=spot_ros.group,
        )
        node.create_service(
            ...
            "estop/hard",
            lambda request, response: spot_ros.service_wrapper(
                "estop/hard", spot_ros.handle_estop_hard, request, response
            ),
            callback_group=spot_ros.group,
        )
        node.create_service(
            ...
            "estop/gentle",
            lambda request, response: spot_ros.service_wrapper(
                "estop/gentle", spot_ros.handle_estop_soft, request, response
            ),
            callback_group=spot_ros.group,
        )
        node.create_service(
            ...
            "estop/release",
            lambda request, response: spot_ros.service_wrapper(
                "estop/release", spot_ros.handle_estop_disengage, request, response
            ),
            callback_group=spot_ros.group,
        )
        ...
        node.create_service(
            ...
            "stair_mode",
            lambda request, response: spot_ros.service_wrapper(
                "stair_mode", spot_ros.handle_stair_mode, request, response
            ),
            callback_group=spot_ros.group,
        )
        node.create_service(
            ...
            "locomotion_mode",
            lambda request, response: spot_ros.service_wrapper(
                "locomotion_mode", spot_ros.handle_locomotion_mode, request, response
            ),
            callback_group=spot_ros.group,
        )
        node.create_service(
            ...
            "max_velocity",
            lambda request, response: spot_ros.service_wrapper(
                "max_velocity", spot_ros.handle_max_vel, request, response
            ),
            callback_group=spot_ros.group,
        )
        node.create_service(
        ...
        node.create_service(
            ...
            "list_graph",
            lambda request, response: spot_ros.service_wrapper(
                "list_graph", spot_ros.handle_list_graph, request, response
            ),
            callback_group=spot_ros.group,
        )
        ...
        node.create_service(
            ...
            spot_ros.handle_graph_nav_upload_graph,
            callback_group=spot_ros.group,
        )
        ...
        node.create_service(
            ...
            spot_ros.handle_graph_nav_clear_graph,
            callback_group=spot_ros.group,
        )
        ...
        node.create_service(
            ...
            spot_ros.handle_graph_nav_get_localization_pose,
            callback_group=spot_ros.group,
        )
        ...
        node.create_service(
            ...
            spot_ros.handle_graph_nav_set_localization,
            callback_group=spot_ros.group,
        )

        spot_ros.navigate_as = ActionServer(
            node,
            NavigateTo,
            "navigate_to",
            spot_ros.handle_navigate_to,
            callback_group=spot_ros.group,
        )
        # spot_ros.navigate_as.start() # As is online

        spot_ros.trajectory_server = ActionServer(
            node,
            Trajectory,
            "trajectory",
            spot_ros.handle_trajectory,
            callback_group=spot_ros.group,
        )
        ...
        if has_arm:
            ...
            spot_ros.robot_command_and_manipulation_servers = (
                SingleGoalMultipleActionServers(
                    node,
                    [
                        (
            ...
            spot_ros.robot_command_server = SingleGoalActionServer(
                node,
                RobotCommand,
                "robot_command",
                spot_ros.handle_robot_command,
        ...
        node.create_timer(
            1 / spot_ros.async_tasks_rate, spot_ros.step, callback_group=spot_ros.group
        )
