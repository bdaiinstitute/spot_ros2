...
TEST_F(RclcppParameterInterfaceEnvVarTest, GetSpotConfigFromParameters) {
  ...
  constexpr auto compress_images = true;
  node_->declare_parameter("compress_images", compress_images);
  ...
  EXPECT_THAT(parameter_interface.getCompressImages(), Eq(compress_images));
...
TEST_F(RclcppParameterInterfaceEnvVarTest, GetConfigDefaults) {
  ...
  EXPECT_THAT(parameter_interface.getCompressImages(), false);
