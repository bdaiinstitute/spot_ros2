...
controller_interface::CallbackReturn FootStateBroadcaster::on_configure(
  ...
  auto get_map_interface_parameter = [&](std::string const& interface, std::string const& interface_to_map) {
    if (std::find(params_.interfaces.begin(), params_.interfaces.end(), interface) != params_.interfaces.end()) {
  ...
  try {
    ...
    realtime_joint_state_publisher_ =
        std::make_shared<realtime_tools::RealtimePublisher<sensor_msgs::msg::JointState>>(joint_state_publisher_);

    dynamic_joint_state_publisher_ = get_node()->create_publisher<control_msgs::msg::DynamicJointState>(
...
controller_interface::CallbackReturn FootStateBroadcaster::on_activate(
    const rclcpp_lifecycle::State& /*previous_state*/) {
  if (!init_joint_data()) {
    RCLCPP_ERROR(get_node()->get_logger(), "Error initializing joint data. FootStateBroadcaster will not run.");
    return CallbackReturn::ERROR;
  }
...
bool FootStateBroadcaster::init_joint_data() {
  joint_names_.clear();
  ...
  for (const auto& name_ifv : name_if_value_mapping_) {
    ...
    if (has_any_key(interfaces_and_values, {HW_IF_POSITION, HW_IF_VELOCITY, HW_IF_EFFORT})) {
      if (!params_.use_urdf_to_filter || !params_.joints.empty() || !is_model_loaded_ ||
  ...
  if (get_node()->get_parameter("extra_joints", extra_joints)) {
    ...
    for (const auto& extra_joint_name : extra_joints_names) {
      if (name_if_value_mapping_.count(extra_joint_name) == 0) {
        name_if_value_mapping_[extra_joint_name] = {{HW_IF_POSITION, 0.0}, {HW_IF_VELOCITY, 0.0}, {HW_IF_EFFORT, 0.0}};
...
void FootStateBroadcaster::init_joint_state_msg() {
...
void FootStateBroadcaster::init_dynamic_joint_state_msg() {
...
bool FootStateBroadcaster::use_all_available_interfaces() const {
...
controller_interface::return_type FootStateBroadcaster::update(const rclcpp::Time& time,
  ...
  if (realtime_joint_state_publisher_ && realtime_joint_state_publisher_->trylock()) {
    ...
    for (size_t i = 0; i < joint_names_.size(); ++i) {
      joint_state_msg.position[i] = get_value(name_if_value_mapping_, joint_names_[i], HW_IF_POSITION);
      joint_state_msg.velocity[i] = get_value(name_if_value_mapping_, joint_names_[i], HW_IF_VELOCITY);
  ...
  if (realtime_dynamic_joint_state_publisher_ && realtime_dynamic_joint_state_publisher_->trylock()) {
    ...
    for (size_t joint_index = 0; joint_index < dynamic_joint_state_msg.joint_names.size(); ++joint_index) {
